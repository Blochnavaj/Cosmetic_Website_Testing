...
const placeOrderRazorpay = async (req, res) => {
  try {
    ...
    if (!userId || !amount || !address || !items) {
    ...
    const options = {
      ...
      currency: 'INR', // Ensure the correct currency
    ...
    res.status(201).json({
      success: true,
      order: razorpayOrder,
      orderData: newOrder // Send back the database order for verification
    });
...
const verifyRazorpayPayment = async (req, res) => {
  try {
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature } = req.body.paymentResponse;
    const { orderId } = req.body; // Use orderId directly for security
    ...
    // ✅ Signature Validation (using backticks for template literals)
